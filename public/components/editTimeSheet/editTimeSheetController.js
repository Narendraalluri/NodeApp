myApp.controller('editTimeSheetController', function($scope, $http, $location,
    ...
    try {
      if ($scope.sheetCount < 4) {
        ...
        for (i = 0; i < $scope.sheetsVal.length; i++) {
          if ($scope.sheetsVal[i].status == undefined || $scope.sheetsVal[
              i].status ==
            false) {
    ...
    for (i = 0; i < data[0].uploadedFiles.length; i++) {
      ...
      $("#UploadedFiles").append('<li id="file_' + data[0].uploadedFiles[
        ...
        '&id=' + data[0].uploadedFiles[i].id + '">' + data[0].uploadedFiles[
          i]
        .name +
        '</a><a href="javascript:removeFile(\'' + data[0].uploadedFiles[
          i].id +
